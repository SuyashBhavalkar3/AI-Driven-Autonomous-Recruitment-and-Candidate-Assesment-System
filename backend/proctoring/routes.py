...
def report_violation(
    ...
    # Non-blocking: update behavioral score incrementally
# (runs on same thread — fast because no model inference, just event accumulation)
    try:
        all_violations = (app.assessment_data or {}).get("violations", []) + \
                        (app.interview_feedback or {}).get("violations", [])
        from dl_engine.event_processor import compute_weak_authenticity_label
        weak_auth = compute_weak_authenticity_label(all_violations)
        # Store running authenticity estimate
        target_payload["live_authenticity_estimate"] = round(weak_auth * 100, 1)
        if data.stage == "assessment":
            app.assessment_data = target_payload
        else:
            app.interview_feedback = target_payload
