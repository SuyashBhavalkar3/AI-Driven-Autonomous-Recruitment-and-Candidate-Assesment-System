...
async def save_profile(
    ...
    if profile_photo:
        ...
        try:
            if not all([os.getenv("CLOUD_NAME"), os.getenv("CLOUDINARY_API_KEY"), os.getenv("CLOUDINARY_API_SECRET")]):
                raise Exception("Cloudinary credentials missing in .env file. Profile photo upload disabled.")
            ...
            # We can either fail or continue without the photo. 
            # Given it's a 500 error reported by user, let's at least log it.
    ...
    try:
        ...
        for exp_data in profile_data.experiences:
            experience = Experience(candidate_id=candidate.id, **exp_data.model_dump())
            db.add(experience)
        
        # Add new education
        for edu_data in profile_data.education:
            education = Education(candidate_id=candidate.id, **edu_data.model_dump())
            db.add(education)
        
        # Add new skills
        for skill_data in profile_data.skills:
            skill = Skill(candidate_id=candidate.id, **skill_data.model_dump())
            db.add(skill)
